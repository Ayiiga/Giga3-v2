...
Branding
- App logo: generated by `cd web && npm run generate:branding` → `web/public/images/logo.png` and synced to `frontend/assets/`.
- Next.js app uses the same asset at `web/public/images/logo.png` (cache-busted via `?v=` in the UI).
- After logo/splash changes: bump runs automatically on `npm run build`; hard-refresh or reinstall the PWA to see new splash/icons on iOS.
