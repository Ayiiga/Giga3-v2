...
export const confirmPurchase = action({
	...
	handler: async (ctx, args) => {
		...
		const email = (session.metadata as { email?: string })?.email || session.customer_details?.email;
...
export const grantPurchaseTokens = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (!user) {
			const userId = await ctx.db.insert("users", { email: args.email, tokens: 0, plan: "paid" });
