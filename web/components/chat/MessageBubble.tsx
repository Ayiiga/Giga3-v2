...
export const MessageBubble = memo(function MessageBubble({
  ...
  return (
    <article
      className={cn(
        "group chat-message-turn w-full min-w-0 max-w-full",
        isUser ? "chat-message-turn-user" : "chat-message-turn-assistant"
      )}
    >
      <div
        className={cn(
          "chat-message-bubble min-w-0 max-w-full",
          isUser
            ? "chat-message-bubble-user w-fit max-w-[min(100%,20rem)] sm:max-w-[min(85%,28rem)]"
            : "w-full"
        )}
      >
        <div className="flex gap-3 sm:gap-3">
          ...
          <div className="min-w-0 max-w-full flex-1">
            <div
              className={cn(
                isUser
                  ? "rounded-[1.25rem] bg-violet-100 px-3.5 py-2 text-zinc-900 shadow-sm dark:bg-violet-950/70 dark:text-zinc-100 sm:px-4 sm:py-2.5"
                  : "chat-assistant-body rounded-2xl bg-zinc-100 px-3.5 py-2.5 text-zinc-900 dark:bg-zinc-800/90 dark:text-zinc-100 sm:bg-transparent sm:px-0 sm:py-1 sm:text-foreground dark:sm:bg-transparent",
