...
describe("service worker cache version", () => {
  it("uses gigaedit-full cache name (v199)", () => {
    const sw = readFileSync(resolve(__dirname, "../../web/public/sw.js"), "utf8");
    expect(sw).toContain('CACHE_NAME = "giga3-shell-v199-gigaedit-full"');
    expect(sw).toContain('NEXT_STATIC_CACHE = "giga3-next-static-v199"');
