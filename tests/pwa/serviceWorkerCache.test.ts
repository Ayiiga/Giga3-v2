...
describe("service worker cache version", () => {
  it("uses pin-newest cache name (v188)", () => {
    const sw = readFileSync(resolve(__dirname, "../../web/public/sw.js"), "utf8");
    expect(sw).toContain('CACHE_NAME = "giga3-shell-v188-pin-newest"');
    expect(sw).toContain('NEXT_STATIC_CACHE = "giga3-next-static-v188"');
