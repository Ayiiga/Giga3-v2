...
describe("gigaSocialMediaPolicy", () => {
  it("accepts videos up to six minutes", () => {
  ...
  it("rejects missing, zero, or over-limit durations", () => {
    expect(() => assertSocialVideoDuration(undefined)).toThrow("6 minutes");
    expect(() => assertSocialVideoDuration(0)).toThrow("6 minutes");
    expect(() => assertSocialVideoDuration(SOCIAL_MAX_VIDEO_DURATION_SEC + 1)).toThrow("6 minutes");
  });
});
